...
def from_csr_2d(val, idx, shape):
    ...
    idx_N, idx_M = idx
    return torch.Tensor(
        csr_matrix((val, idx_M, idx_N), shape=shape).toarray()
    ).float()
...
def to_sparse_format_conv2d(dense_weight):
    ...
    qsppb.sparsify_conv2d(OC, IC, K, dense_weight, W_OC, W_IC, W_X, W_Y, W_val)
...
def from_sparse_format_conv2d(W_val, W_idx, shape):
    ...
    qsppb.densify_conv2d(OC, IC, K, dense_weight, W_OC, W_IC, W_X, W_Y, W_val)
