...
if __name__ == '__main__':
    ...
    stride = 1  # stride
    padding = 0  # padding
    vectorizing_over_on = True  # as described in the paper
    ...
    our_X.retain_grad()
    our_Y = Y_orig.clone()
    spconv = QuantizedSparseConv2d(W, bias=torch.nn.Parameter(deepcopy(bias)), padding=padding, stride=stride, vectorizing_over_on=vectorizing_over_on)
    our_O = spconv(our_X)
    ...
    print(torch_X_grad.size(), our_X_grad.size())
    """