...
class SparseLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, inputT, W_val, W_idx, bias, N):
        ...
        qsppb.uq_sparse_linear_vectorized_forward(input_flat, W_idx_N, W_idx_M, W_val, output)
    ...
    def backward(ctx, grad_output_t):
        ...
        qsppb.uq_sparse_linear_vectorized_backward(
            input_flat,
            W_idx_N,
            W_idx_M,
            W_val,
...
class SparseConvFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input, W_val, W_idx, bias, OC, K, padding, stride, vectorizing_fwd_over_on,
                vectorizing_bwd_over_on):
        orig_input = input
        ...
        if vectorizing_fwd_over_on:
            ...
            qsppb.uq_sparse_conv2d_vectorized_forward_over_on_stride_1(input, *W_idx, W_val, output, K, padding)
        else:
            input = input.permute(1, 2, 3, 0).contiguous()
            output = torch.zeros(OC, OM, ON, B).float()
            if stride == 1:
                qsppb.uq_sparse_conv2d_vectorized_forward_stride_1(input, *W_idx, W_val, output, K, padding)
            elif stride == 2:
                qsppb.uq_sparse_conv2d_vectorized_forward_stride_2(input, *W_idx, W_val, output, K, padding)
    ...
    def backward(ctx, grad_output):
        ...
        if vectorizing_bwd_over_on:
            if stride == 1:
                qsppb.uq_sparse_conv2d_vectorized_backward_over_on_stride_1(input, W_idx_OC, W_idx_IC, W_idx_X, W_idx_Y,
                                                                        W_val, grad_output, grad_input, grad_W_val, K,
                                                                        padding)
            else:
                qsppb.uq_sparse_conv2d_vectorized_backward_over_on_stride_2(input, W_idx_OC, W_idx_IC, W_idx_X, W_idx_Y,
                                                                        W_val, grad_output, grad_input, grad_W_val, K,
                                                                        padding)
        else:
            go = grad_output.permute(1, 2, 3, 0).contiguous()
            if stride == 1:
                qsppb.uq_sparse_conv2d_vectorized_backward_stride_1(input, W_idx_OC, W_idx_IC, W_idx_X, W_idx_Y, W_val, go,
                                                                grad_input, grad_W_val, K, padding)
            else:
                qsppb.uq_sparse_conv2d_vectorized_backward_stride_2(input, W_idx_OC, W_idx_IC, W_idx_X, W_idx_Y, W_val, go,
                                                                grad_input, grad_W_val, K, padding)
