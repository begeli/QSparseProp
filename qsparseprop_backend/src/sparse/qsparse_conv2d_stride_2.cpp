#include "qsparse_conv2d_stride_2.h"

void sparse::quantized_sparse_conv2d_vectorized_forward_stride_2(
    ...
    for (int oc = 0; oc < OC; oc++) {
        ...
        for (int ic = 0; ic < IC; ic++){
            ...
            for (int si = ic_s; si < ic_e; si++) {
                ...
                const int px_start_tmp_0 = 2 * p_start;
                const int px_start_tmp_1 = px_start_tmp_0 - padding;
                const int px_start = px_start_tmp_1 + i;

                for (int po = p_start, px = px_start; po < p_end; po++, px += 2) {
                    int qo = q_start;
                    int qx_start_tmp_0 = 2 * q_start;
                    int qx_start_tmp_1 = qx_start_tmp_0 - padding;
                    int qx = qx_start_tmp_1 + j;
...
void sparse::quantized_sparse_conv2d_vectorized_parallel_forward_stride_2(
    int B, int IC, int OC, int M, int N, int K, int W_nnz, int padding, int8_t* X,
    float X_scale, int* W_idx_OC, int16_t* W_idx_IC, uint8_t* W_idx_X,
    uint8_t* W_idx_Y, int8_t* W_val, float W_scale, float* O
) {
#if defined(_OPENMP)
    const int OM = (int) ceil((float) (M + 2 * padding - K + 1) / 2);
...
void sparse::quantized_grouped_sparse_conv2d_vectorized_forward_stride_2(
    int B, int IC, int OC, int M, int N, int K, int W_nnz, int padding,
...
void sparse::quantized_grouped_sparse_conv2d_vectorized_parallel_forward_stride_2(
    int B, int IC, int OC, int M, int N, int K, int W_nnz, int padding,
    int8_t* X, float* X_scale, int X_qgroup_size, int* W_idx_OC, int16_t* W_idx_IC, uint8_t* W_idx_X,
    uint8_t* W_idx_Y, int8_t* W_val, float* W_scale, int W_qgroup_size, float* O
) {
...
void sparse::quantized_sparse_conv2d_vectorized_backward_stride_2(
    int B, int IC, int OC, int M, int N, int K, int W_nnz, int padding,
    int8_t* X, float X_scale, int* W_idx_OC, int16_t* W_idx_IC, uint8_t* W_idx_X,
...
void sparse::quantized_sparse_conv2d_vectorized_parallel_backward_stride_2(
    int B, int IC, int OC, int M, int N, int K, int W_nnz, int padding, int8_t* X,
    float X_scale, int* W_idx_OC, int16_t* W_idx_IC, uint8_t* W_idx_X,
    uint8_t* W_idx_Y, int8_t* W_val, float W_scale, int8_t* dLdO,
    float dLdO_scale, float* dLdX, float* dLdW_val
) {
#if defined(_OPENMP)
    const int OM = (int) ceil((float) (M + 2 * padding - K + 1) / 2);
    const int ON = (int) ceil((float) (N + 2 * padding - K + 1) / 2);
...
void sparse::quantized_grouped_sparse_conv2d_vectorized_backward_stride_2(
    int B, int IC, int OC, int M, int N, int K, int W_nnz, int padding,
    int8_t* X, float* X_scale, int X_qgroup_size, int* W_idx_OC, int16_t* W_idx_IC, uint8_t* W_idx_X,
...
void sparse::quantized_grouped_sparse_conv2d_vectorized_parallel_backward_stride_2(
    int B, int IC, int OC, int M, int N, int K, int W_nnz, int padding,
    int8_t* X, float* X_scale, int X_qgroup_size, int* W_idx_OC, int16_t* W_idx_IC, uint8_t* W_idx_X,
    uint8_t* W_idx_Y, int8_t* W_val, float* W_scale, int W_qgroup_size,
    int8_t * dLdO, float* dLdO_scale, int dLdO_qgroup_size, float* dLdX, float* dLdW_val
